...
def generate_centrality_plots(base_dir):
    ...
    # Parse method names (if they are string representations of lists)
    methods = []
    ...
    for method_name, dir_name in methods:
        ...
        for phase in phases:
            ...
            centrality_values['Betweenness'].extend(betweenness.values())
            centrality_values['Closeness'].extend(closeness.values())
            centrality_values['Eigenvector'].extend([v for v in eigenvector.values() if not np.isnan(v)])

    # Check if any centrality data was collected
    if not any(centrality_values['Betweenness']):
        print("No centrality data collected. Exiting.")
        return

    # Compute global min and max for each centrality measure for consistent coloring
    global_min_max = {}
    for measure in ['Betweenness', 'Closeness', 'Eigenvector']:
        values = centrality_values[measure]
        if values:
    ...
    for measure in ['Betweenness', 'Closeness', 'Eigenvector']:
        ...
        for i, phase in enumerate(phases):
            for j, (method_name, dir_name) in enumerate(methods):
                ...
                nodes = nx.draw_networkx_nodes(G, positions, ax=ax, node_color=node_colors, cmap='viridis',
                                               vmin=global_min_max[measure][0],
                                               vmax=global_min_max[measure][1], node_size=50)
                # Remove axis
                ax.axis('off')
                # Set column titles
                if i == 0:
