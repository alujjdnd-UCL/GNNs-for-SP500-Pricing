...
from src.models.TGCNCell import TGCNCell
...
class A3TGCN(nn.Module):
    ...
    def forward(self, x: torch.tensor, edge_index: torch.tensor, edge_weight: torch.tensor) -> torch.tensor:
        ...
        h_final = torch.zeros(x.shape[0], x.shape[-1], self.hidden_size, device=device)
        for t in range(x.shape[-1]):
            h = x[:, :, t].to(device)  # h is the output of the previous GRU layer (the input features for the first layer)
            for i, cell in enumerate(self.cells):
                h = cell(h, edge_index, edge_weight, h_prev[i])
                h_prev[i] = h
            h_final[:, t, :] = h
        h_final = h_final.to(device)
        attention_out = self.attention(h_final).to(device)
        return self.out(F.leaky_relu(torch.sum(F.leaky_relu(h_final) * attention_out, dim=1)))
